...
def parse_args():
    ...
    parser.add_argument("--mode", type=str, choices=["langgraph", "baseline", "langchain_aws", "all"], default="langgraph", 
                        help="Benchmark mode")
...
def main():
    ...
    logger.info(f"Mode: {args.mode}, Concurrency={args.concurrency}, Limit={args.limit}")
    ...
    try:
        if args.mode in ["langgraph", "all"]:
            ...
            df_map["langgraph"] = asyncio.run(runner.run(
        ...
        if args.mode in ["baseline", "all"]:
            ...
            df_map["baseline"] = asyncio.run(run_baseline_benchmark(
                prompts_path=args.data,
        ...
        if args.mode in ["langchain_aws", "all"]:
            ...
            df_map["langchain_aws"] = asyncio.run(run_langchain_aws_benchmark(
                prompts_path=args.data,
                output_path="measurements", # Base dir
                concurrency=args.concurrency,
                limit=args.limit
        ...
        if args.mode == "all" and len(df_map) > 1:
            ...
            comparison_path = get_report_path("measurements", "comparison", extension="csv")
