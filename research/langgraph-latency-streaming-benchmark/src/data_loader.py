...
class DataLoader:
    def __init__(self, data_path: str = "data/mtbench_prompts.parquet"):
    ...
    def _categorize_complexity(self, text: str, category: str) -> str:
        ...
        Low: Writing category (usually shorter) or length < 50 tokens (approx words * 1.3)
    ...
    def load_prompts(self) -> Dict[str, List[PromptSample]]:
        ...
        try:
            ...
            logger.info(f"Categorization results: Low={len(samples['Low'])}, Medium={len(samples['Medium'])}, High={len(samples['High'])}")
            return samples
