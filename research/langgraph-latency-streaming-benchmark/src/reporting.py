...
class ReportGenerator:
    ...
    def log_summary(df: pd.DataFrame) -> None:
        ...
        metrics = MetricsCalculator.calculate_percentiles(
            valid["e2e_latency_ms"].tolist()
        )
        ...
        if "complexity" in df.columns:
            grouped = valid.groupby("complexity")["e2e_latency_ms"].describe(
            ...
            with open(md_path, "w", encoding="utf-8") as f:
                ...
                for pid in unique_ids:
                    ...
                    for mode in df_map.keys():
                        ...
                        vals.append(
                            f"{v[0]:.2f}" if len(v) > 0 and v[0] is not None else "N/A"
                        )
                    row += " | ".join(vals) + " |\n"
                    f.write(row)
